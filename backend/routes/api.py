...
def dashboard_stats_v2():
    ...
    # Use logs count as a proxy for total predictions if no explicit table exists
    total_predictions = MonitoringLog.query.count() * 100
    
    logs = MonitoringLog.query.order_by(MonitoringLog.timestamp.desc()).limit(30).all()
    accuracy_trend = []
    avg_accuracy = 0.0
    
    if logs:
        valid_acc = [l.accuracy for l in logs if l.accuracy is not None]
        if valid_acc:
            avg_accuracy = sum(valid_acc) / len(valid_acc)
